...
import School_Marksheet_Frontend
       ...
       def __init__(self, master):
              ...
              def Search():
                     if(len(self.roll.get()) != 0):
                           row = School_Marksheet_Backend.search(int(self.roll.get()))
                           print(row)
                           School_Marksheet_Frontend.search_result_marksheet(row)
                     else:
                            tkinter.messagebox.askokcancel('Attention','Please enter valid Roll No.')
                            return

              def new():
                     filename = 'School_Marksheet_Frontend.py'
                     os.system(filename)
